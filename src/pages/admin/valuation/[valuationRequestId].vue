...
const isQueued = computed(() => valuationRequest.value?.status === 'queued')
...
async function startReview() {
    ...
    try {
        valuationRequest.value = await api.startValuationReview(
            ...
            <section
                ...
                <div class="p-6">
                    ...
                    <button
                        v-if="isQueued"
                        :disabled="isStarting"
                        class="mt-5 bg-(--primary) px-5 py-3 text-sm text-(--primary-foreground) disabled:opacity-60"
                        @click="startReview">
                        {{ isStarting ? 'Starting...' : 'Start review' }}
                    </button>
                    <button
                        v-if="isReviewing"
                        class="mt-5 bg-(--primary) px-5 py-3 text-sm text-(--primary-foreground)"
                        @click="router.push(reviewPath)">
                        Continue review
                    </button>
                    <section
                        v-if="isCompleted"
